...
from constants import SSE_TIMEOUT_SECONDS, time_ns, mask_api_key
...
async def generate_chat_completion(
    ...
    # Get model config (would query DB in production)
...
async def _get_model_config(model_id: str) -> dict:
    ...
    Flow:
    1. Parse model_id to extract provider prefix (e.g., "openai.gpt-4o" -> "openai")
    2. Query providers table for matching active provider
    3. Fetch model details from provider's /models endpoint
    4. Return enriched config with capabilities, params, filters
    """
